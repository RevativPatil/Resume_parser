...
- **70% Minimum Match Filter**: Only displays candidates with ≥70% skill match to your requirements
...
- Click Search
- Results show matching candidates with ≥70% skill match only
- Candidates with <70% match are automatically filtered out
- Results are sorted by match percentage (highest first)

### 3. Search by Job Role
- Enter any job role (e.g., "MERN Stack Developer")
- Click Search
- Results show only candidates with ≥70% match to required skills
- Results are ranked by skill match percentage (highest first)
- **Important**: Only candidates matching 70% or more of required skills are displayed
...
### Search Endpoints
- `GET /api/search?q=<skill>` - Search by skill (filters ≥70% match)
- `GET /api/search-by-role?role=<role_name>` - Search by job role (filters ≥70% match)
- `GET /api/job-roles` - Get all available job roles

**Note**: All search endpoints automatically filter out candidates with less than 70% skill match
...
6. **Understand the 70% filter** - Only high-quality matches (≥70%) are returned to save time reviewing unqualified candidates
...
3. **Searches for "MERN Stack Developer"**
   - System shows only candidates with ≥70% skill match
   - Example: 2 candidates qualify (80% and 75% match)
   - Candidates with 65%, 55%, 45%, 35%, 25%, 15% are filtered out

4. **Clicks on top candidate (80% match)**
   - Views resume PDF
   - Sees extracted information
   - Understands which skills match and which are missing

5. **Interviews candidate and hires them**
   - Knows candidate has 80% of required skills
   - Can plan training for missing 20%
...
## ✨ 70% Skill Match Filter

**How It Works:**
- When you search for skills or job roles, the system calculates match percentages
- **Only candidates with ≥70% match are displayed**
- Candidates with <70% match are automatically filtered out
- Results are sorted by match percentage (highest first)

**Why 70%?**
- Ensures only qualified candidates are shown
- Reduces time spent reviewing unqualified resumes
- Focuses HR team on high-potential candidates
- Maintains quality standards for candidate screening

**Example:**
- Searching for "MERN Stack Developer" (18 required skills)
- Candidate A: 15/18 skills = 83% ✅ (displayed)
- Candidate B: 13/18 skills = 72% ✅ (displayed)
- Candidate C: 12/18 skills = 67% ❌ (filtered out)
- Candidate D: 9/18 skills = 50% ❌ (filtered out)
